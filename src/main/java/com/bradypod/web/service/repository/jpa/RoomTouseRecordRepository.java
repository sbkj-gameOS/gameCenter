...
public abstract interface RoomTouseRecordRepository extends JpaRepository<RoomTouseRecord, String> {
     ...
     * @return
     */
    @Query(value = "select * from bm_room_touse_record where USER_NAME LIKE  CONCAT('%',:userName,'%') and INVITATION_CODE LIKE CONCAT('%',:invitationCode,'%')  limit :startData,:limit",nativeQuery = true)
